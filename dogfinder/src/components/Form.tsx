...
const Form = ({ nameError, emailError, handleChange, handleClick, onSubmit }: IFormProps) => {
    return (
        <form className="w-3/4 md:w-3/5 lg:w-2/5 h-1/2 flex flex-col items-center justify-center bg-slate-50 relative top-1/4 p-4 rounded-md shadow-md shadow-teal-500" id="login-form" role="login" onSubmit={onSubmit}>
            ...
            <Input type="text" role="name" styles="rounded-sm border border-slate-800 outline-teal-700 p-1" onChange={handleChange} required={true} />
            <label className="mt-4" htmlFor="">Email</label>
            <Input type="email" role="email" styles="rounded-sm border border-slate-800 outline-teal-700 p-1" onChange={handleChange} required={true} />
            <Button styles="mt-4 p-2 w-20 bg-teal-400 font-medium rounded-sm" text="Login" onClick={handleClick}></Button>
